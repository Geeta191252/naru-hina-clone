...
import requests
from info import *
from info import FTM_AUTOINDEX
from utils import *
...
async def media(bot, message):
    """Media Handler"""
    if FTM_AUTOINDEX and message.chat.id in FTM_AUTOINDEX:
        return
...
@Client.on_message(filters.chat(FTM_AUTOINDEX) & media_filter)
async def ftm_autoindex_handler(bot, message):
    """FTM Auto Index Handler - Only indexes if bot is admin"""
    if not FTM_AUTOINDEX:
        return
    try:
        chat_member = await bot.get_chat_member(message.chat.id, bot.me.id)
        if chat_member.status not in [enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER]:
            return
    except Exception as e:
        LOGGER.error(f"FTM AutoIndex: Failed to check admin status in {message.chat.id}: {e}")
        return
    
    for file_type in ("document", "video"):
        media_file = getattr(message, file_type, None)
        if media_file is not None:
            break
    else:
        return
    
    file_name = media_file.file_name or ""
    video_extensions = ('.mp4', '.mkv', '.avi', '.mov', '.wmv', '.flv', '.webm', '.m4v', '.3gp')
    if file_type == "document" and not file_name.lower().endswith(video_extensions):
        return
    
    media_file.file_type = file_type
    media_file.caption = message.caption
    success, result = await save_file(media_file)
    if success:
        LOGGER.info(f"FTM AutoIndex: Indexed {file_name} from {message.chat.title or message.chat.id}")
        try:
            if result == 1 and await get_status(bot.me.id):
                await send_movie_update(bot, file_name=media_file.file_name, caption=media_file.caption)
        except Exception as e:
            LOGGER.error(f"FTM AutoIndex: Movie update error - {e}")

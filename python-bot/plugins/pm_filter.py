...
async def give_filter(client, message):
    ...
    try:
        ...
        LOGGER.error(f"[give_filter] silentdb.update_top_messages failed (ignored): {e}")
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        if settings['auto_ffilter']:
            ...
            # Force Add Check - Skip for admins
            if message.from_user.id not in ADMINS:
                forceadd_enabled = await db.is_forceadd_enabled(message.chat.id)
                if forceadd_enabled:
                    user_add_count = await db.get_user_add_count(message.chat.id, message.from_user.id)
                    required_count = await db.get_forceadd_count(message.chat.id)
                    if user_add_count < required_count:
                        remaining = required_count - user_add_count
                        try:
                        ...
                        invite_link = await db.get_user_invite_link(message.chat.id, message.from_user.id)
                        logger.info(f"[ForceAdd] Existing invite link for user {message.from_user.id}: {invite_link}")
                        if not invite_link:
                            try:
                                invite_obj = await client.create_chat_invite_link(
                                    message.chat.id,
                                    name=f"ForceAdd_{message.from_user.id}"
                                )
                                invite_link = invite_obj.invite_link
                                logger.info(f"[ForceAdd] Created new invite link for user {message.from_user.id}: {invite_link}")
                                await db.save_user_invite_link(message.chat.id, message.from_user.id, invite_link)
                            except Exception as e:
                                logger.error(f"[ForceAdd] Error creating invite link: {e}")
                                invite_link = None
                        
                        warn_text = (
                            f"❌ <b>ʜᴇʏ {message.from_user.mention}!</b>\n"
                            f"ʏᴏᴜ ɴᴇᴇᴅ ᴛᴏ ᴀᴅᴅ <b>{remaining}</b> ᴍᴏʀᴇ ᴍᴇᴍʙᴇʀꜱ ᴛᴏ ᴛʜɪꜱ ɢʀᴏᴜᴘ ᴛᴏ ꜱᴇᴀʀᴄʜ ꜰᴏʀ ᴍᴏᴠɪᴇꜱ!\n"
                            f"📊 <b>ʏᴏᴜʀ ᴘʀᴏɢʀᴇꜱꜱ:</b> {user_add_count}/{required_count}\n"
                            f"✅ <b>ʜᴏᴡ ᴛᴏ ᴀᴅᴅ ᴍᴇᴍʙᴇʀꜱ:</b>\n"
                            f"• ᴀᴅᴅ ᴍᴇᴍʙᴇʀꜱ ᴅɪʀᴇᴄᴛʟʏ ᴛᴏ ᴛʜɪꜱ ɢʀᴏᴜᴘ\n"
                            f"• ᴏʀ ᴜꜱᴇ ᴛʜᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇 ᴛᴏ ꜱʜᴀʀᴇ ɪɴᴠɪᴛᴇ ʟɪɴᴋ\n"
                            f"━━━━━━━━━━━━━━━━━━━━━\n"
                            f"❌ <b>अरे {message.from_user.mention}!</b>\n"
                            f"Movies search करने के लिए आपको इस group में <b>{remaining}</b> और members add करने होंगे!\n"
                            f"📊 <b>आपकी Progress:</b> {user_add_count}/{required_count}\n\n"
                            f"✅ <b>Members कैसे add करें:</b>\n"
                            f"• Group में directly members add करें\n"
                            f"• या नीचे 👇 button पर click करके invite link share करें\n"
                            f"• जब कोई आपके link से join करेगा, count automatically बढ़ जाएगा!"
                        )
                        
                        if invite_link:
                            share_url = f"https://t.me/share/url?url={quote_plus(invite_link)}&text={quote_plus('Join this group for movies!')}"
                            buttons = InlineKeyboardMarkup([
                                [InlineKeyboardButton("👥 ɪɴᴠɪᴛᴇ ꜰʀɪᴇɴᴅꜱ", url=share_url)]
                            ])
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        settings = await get_settings(grp_id)
        btn2 = [[
                 InlineKeyboardButton("ᴄʜᴇᴄᴋ ᴍʏ ᴅᴍ 🗳️", url=f"https://t.me/{temp.U_NAME}")
               ]]
        ...
        try:
            ...
            silent_download = f"{URL}{str(silent_msg.id)}/{quote_plus(get_name(silent_msg))}?hash={get_hash(silent_msg)}&download=1"
            btn= [[
        ...
        buttons = [[
            InlineKeyboardButton ('sᴏᴜʀᴄᴇ ᴏᴡɴᴇʀ', callback_data='source'),
        ],[
            InlineKeyboardButton('⇋ ʙᴀᴄᴋ ᴛᴏ ʜᴏᴍᴇ ⇋', callback_data='start')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        ...
        buttons = [[
            InlineKeyboardButton('ꜱᴏᴜʀᴄᴇ ᴏᴡɴᴇʀ', url="https://t.me/Hidden_Xman"),
